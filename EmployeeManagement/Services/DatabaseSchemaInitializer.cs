...
BEGIN
    EXEC('UPDATE TaskItem SET Status = CASE
        WHEN PlannedEndDate < CAST(GETDATE() AS date) THEN ''Completed''
        WHEN PlannedStartDate <= CAST(GETDATE() AS date) AND PlannedEndDate >= CAST(GETDATE() AS date) THEN ''InProgress''
        WHEN PlannedStartDate > CAST(GETDATE() AS date) THEN ''Ready''
        ELSE ''Backlog'' END');
END;
...
BEGIN
    ...
    CREATE INDEX IX_AuditLog_ProjectId ON AuditLog(ProjectId);
END;
""";
