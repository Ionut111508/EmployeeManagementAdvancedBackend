...
public class WorkflowTests
{
    ...
    [Fact]
    public async Task PlannedTaskCreation_GeneratesTaskAndDescriptionIds()
    {
        await using var context = CreateContext();
        context.Projects.Add(new Project { ProjectId = "P1", ProjectName = "Project" });
        await context.SaveChangesAsync();
        ...
        var controller = new TasksController(
            ...
            allocationService,
            new AccessScopeService(context),
            new AuditLogService(context))
        {
            ControllerContext = new ControllerContext { HttpContext = new DefaultHttpContext { User = User("ADMIN", RoleNames.Admin) } }
        };
        var startDate = DateTime.Today.AddDays(1);

        var action = await controller.CreatePlanned(new CreatePlannedTaskRequest
        {
            ProjectId = "P1",
            TaskName = "Generated identifiers",
            EstimatedHours = 8,
            DescriptionText = "Identifiers are generated by the backend.",
        ...
        var response = Assert.IsType<CreatePlannedTaskResponse>(created.Value);
        Assert.StartsWith("TSK_", response.Task.TaskId);
        Assert.StartsWith("DESC_", response.Task.DescriptionId);
        Assert.Equal(response.Task.TaskId, (await context.TaskItems.SingleAsync()).TaskId);
        Assert.Equal(response.Task.DescriptionId, (await context.Descriptions.SingleAsync()).DescriptionId);
    }
